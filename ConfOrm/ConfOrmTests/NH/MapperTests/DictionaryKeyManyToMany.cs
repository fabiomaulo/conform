...
namespace ConfOrmTests.NH.MapperTests
{
	public class DictionaryKeyManyToMany
	{
		private class Person
		{
			...
			public IDictionary<Animal, string> Pets { get; set; }
		}
		...
		private Mock<IDomainInspector> GetMockedDomainInspector()
		{
			...
			orm.Setup(m => m.IsDictionary(It.Is<MemberInfo>(p => p == typeof(Person).GetProperty("Pets")))).Returns(true);
			return orm;
		}
		...
		private void VerifyMapping(HbmMapping mapping)
		{
			...
			collection.Item.Should().Be.OfType<HbmMapKeyManyToMany>();
			var keyRelation = (HbmMapKeyManyToMany)collection.Item;
			keyRelation.Class.Should().Not.Be.Null();
			keyRelation.Class.Should().Contain("Animal");
		}
