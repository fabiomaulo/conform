...
{
	public class HierarchyTablePerConcreteClassJumpingImpOfInterfaceTest
	{
		private interface IEntitySimple
		{
			int Id { get; set; }
			string Name { get; set; }
		}

		private class EntitySimple : IEntitySimple
		{
		...
		private Mock<IDomainInspector> GetMockedDomainInspector()
		{
			...
			orm.Setup(m => m.IsEntity(It.Is<Type>(t => t != typeof(Hinherited) && t != typeof(EntitySimple) && t != typeof(object)))).Returns(true);
			orm.Setup(m => m.IsRootEntity(It.Is<Type>(t => t == typeof(IEntitySimple)))).Returns(true);
		...
		public void MappingsShouldContainOnlyASubclass()
		{
			...
			mapping.UnionSubclasses.Should().Have.Count.EqualTo(1);
		}
		...
		public void TheBaseClassShouldNotBeTheNotEntity()
		{
			...
			var domainInspector = orm.Object;
			HbmMapping mapping = GetMapping(domainInspector);
			var hbmclass = mapping.UnionSubclasses.Single();
			hbmclass.extends.Should().Contain("IEntitySimple");
		}
