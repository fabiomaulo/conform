...
{
	public class SetOfManyToManyTest
	{
		private class Person
		{
			public int Id { get; set; }
			public ISet<Animal> Pets { get; set; }
		}
		...
		private Mock<IDomainInspector> GetMockedDomainInspector()
		{
			...
			orm.Setup(m => m.IsSet(It.Is<MemberInfo>(p => p == typeof(Person).GetProperty("Pets")))).Returns(true);
			return orm;
		}
		...
		private void VerifyMapping(HbmMapping mapping)
		{
			...
			collection.ElementRelationship.Should().Be.OfType<HbmManyToMany>();
			var elementRelation = (HbmManyToMany)collection.ElementRelationship;
