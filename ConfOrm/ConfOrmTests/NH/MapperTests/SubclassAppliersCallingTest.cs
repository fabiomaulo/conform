...
{
	public class SubclassAppliersCallingTest
	{
		...
		public void ApplierCalledPerSubclass()
		{
			...
			var applier = new Mock<IPatternApplier<Type, ISubclassAttributesMapper>>();
			applier.Setup(x => x.Match(It.IsAny<Type>())).Returns(true);

			mapper.PatternsAppliers.Subclass.Add(applier.Object);
			...
			applier.Verify(x => x.Apply(It.Is<Type>(t => t == typeof(Inherited)), It.Is<ISubclassAttributesMapper>(cm => cm != null)), Times.Once());
		}
	}
}