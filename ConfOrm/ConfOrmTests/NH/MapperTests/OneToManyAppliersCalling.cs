...
namespace ConfOrmTests.NH.MapperTests
{
	public class OneToManyAppliersCalling
	{
		...
		public void WhenRegisteredApplierForBagThenCallApplyForMemberInfo()
		{
			...
			oneToManyApplier.Setup(x => x.Match(It.Is<MemberInfo>(mi => mi.Equals(ConfOrm.ForClass<Person>.Property(p => p.Pets))))).Returns(true);

			mapper.PatternsAppliers.OneToMany.Add(oneToManyApplier.Object);

			mapper.CompileMappingFor(new[] { typeof(Person) });

			oneToManyApplier.Verify(x => x.Apply(It.Is<MemberInfo>(mi => mi.Equals(ConfOrm.ForClass<Person>.Property(p => p.Pets))), It.IsAny<IOneToManyMapper>()));
		}
		...
		public void WhenRegisteredApplierForBagThenCallApplyForPropertyPath()
		{
			...
			oneToManyApplier.Setup(x => x.Match(It.Is<PropertyPath>(pp => pp.Equals(new PropertyPath(null, ConfOrm.ForClass<Person>.Property(p => p.Pets)))))).Returns(true);

			mapper.PatternsAppliers.OneToManyPath.Add(oneToManyApplier.Object);

			mapper.CompileMappingFor(new[] { typeof(Person) });

			oneToManyApplier.Verify(x => x.Apply(It.Is<PropertyPath>(pp => pp.Equals(new PropertyPath(null, ConfOrm.ForClass<Person>.Property(p => p.Pets)))), It.IsAny<IOneToManyMapper>()));
		}
	}
}